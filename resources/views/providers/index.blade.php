...
@section('title', __('system.providers'))
...
@section('content_header')
    <cw-header-title>
        ...
        <x-adminlte-button label="{{__('system.add_provider')}}" data-toggle="modal" data-target="#modalAdd" class="bg-success" icon="fas fa-plus" id="openModalAdd"/>
...
@section('content')
    <x-adminlte-card theme="success" theme-mode="outline">
        ...
        foreach ($providers as $key => $provider) {
            ...
            $data[$key]['actions'] =
            "<nobr>
                <button class='btn btn-xs btn-default text-primary mx-1 shadow btnAction edit' title='".$system_edit."' data-id='".$provider["id"]."'>
                    <i class='fa fa-lg fa-fw fa-pen'></i>
                </button>
                <button class='btn btn-xs btn-default text-danger mx-1 shadow btnAction delete' title='".$system_delete."' data-id='".$provider["id"]."'>
                    <i class='fa fa-lg fa-fw fa-trash'></i>
                </button>
                <button class='btn btn-xs btn-default text-teal mx-1 shadow btnAction details' title='".$system_details."' data-id='".$provider["id"]."'>
        ...
        {{-- Modal ADD --}}
        <x-adminlte-modal id="modalAdd" title=" {{ __('system.add_provider') }}" size="lg" theme="success" icon="fas fa-truck" v-centered static-backdrop scrollable>
            <form action="{{route('providers.store')}}" method="post" enctype="multipart/form-data" id="form_add_provider">
                @csrf
        ...
        {{-- Modal EDIT --}}
        <x-adminlte-modal id="modalEdit" title=" {{ __('system.edit_provider') }}" size="lg" theme="success" icon="fas fa-truck" v-centered static-backdrop scrollable>
...
@section('js')
    <script>
        ...
        const edit_provider = (provider) => {
            ...
            document.querySelector('#form_edit_provider').setAttribute('action', route_edit.replace('provider_id', provider.id))
