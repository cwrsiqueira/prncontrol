...
@section('title', __('system.materials'))
...
@section('content_header')
    <cw-header-title>
        ...
        <x-adminlte-button label="{{__('system.add_material')}}" data-toggle="modal" data-target="#modalAdd" class="bg-success" icon="fas fa-plus" id="openModalAdd"/>
        <x-adminlte-button data-toggle="modal" data-target="#modalEdit" id="openModalEdit" style="display:none;"/>
    </cw-header-title>
@stop

@section('content')
    <x-adminlte-card theme="success" theme-mode="outline" icon="fas fa-boxes">
        ...
        foreach ($materials as $key => $material) {
            $data[$key]['id'] = $material['id'];
            $data[$key]['name'] = $material['name'];
            $data[$key]['obs'] = $material['obs'];
            $data[$key]['actions'] =
            "<nobr>
                <button class='btn btn-xs btn-default text-primary mx-1 shadow btnAction edit' title='".$system_edit."' data-id='".$material["id"]."'>
                    <i class='fa fa-lg fa-fw fa-pen'></i>
                </button>
                <button class='btn btn-xs btn-default text-danger mx-1 shadow btnAction delete' title='".$system_delete."' data-id='".$material["id"]."'>
                    <i class='fa fa-lg fa-fw fa-trash'></i>
                </button>
                <button class='btn btn-xs btn-default text-teal mx-1 shadow btnAction details' title='".$system_details."' data-id='".$material["id"]."'>
        ...
        {{-- Modal ADD --}}
        <x-adminlte-modal id="modalAdd" title=" {{ __('system.add_material') }}" size="lg" theme="success" icon="fas fa-boxes" v-centered static-backdrop scrollable>
            <form action="{{route('materials.store')}}" method="post" enctype="multipart/form-data" id="form_add_material">
                @csrf
        ...
        {{-- Modal EDIT --}}
        <x-adminlte-modal id="modalEdit" title=" {{ __('system.edit_material') }}" size="lg" theme="success" icon="fas fa-boxes" v-centered static-backdrop scrollable>
...
@section('js')
    <script>
        ...
        const edit_material = (material) => {
            ...
            document.querySelector('#form_edit_material').setAttribute('action', route_edit.replace('material_id', material.id))
