...
@section('title', __('system.constructions'))
...
@section('content_header')
    <cw-header-title>
        ...
        <x-adminlte-button label="{{__('system.add_construction')}}" data-toggle="modal" data-target="#modalAdd" class="bg-success" icon="fas fa-plus" id="openModalAdd"/>
        <x-adminlte-button data-toggle="modal" data-target="#modalEdit" id="openModalEdit" style="display:none;"/>
    </cw-header-title>
@stop

@section('content')
    <x-adminlte-card theme="success" theme-mode="outline" icon="fas fa-store-alt">
        ...
        foreach ($constructions as $key => $construction) {
            $data[$key]['id'] = $construction['id'];
            $data[$key]['name'] = $construction['name'];
            $data[$key]['obs'] = $construction['obs'];
            $data[$key]['actions'] =
            "<nobr>
                <button class='btn btn-xs btn-default text-primary mx-1 shadow btnAction edit' title='".$system_edit."' data-id='".$construction["id"]."'>
                    <i class='fa fa-lg fa-fw fa-pen'></i>
                </button>
                <button class='btn btn-xs btn-default text-danger mx-1 shadow btnAction delete' title='".$system_delete."' data-id='".$construction["id"]."'>
                    <i class='fa fa-lg fa-fw fa-trash'></i>
                </button>
                <button class='btn btn-xs btn-default text-teal mx-1 shadow btnAction details' title='".$system_details."' data-id='".$construction["id"]."'>
        ...
        {{-- Modal ADD --}}
        <x-adminlte-modal id="modalAdd" title=" {{ __('system.add_construction') }}" size="lg" theme="success" icon="fas fa-store-alt" v-centered static-backdrop scrollable>
            <form action="{{route('constructions.store')}}" method="post" enctype="multipart/form-data" id="form_add_construction">
                @csrf
                ...
                <div class="row">
                    <x-adminlte-input name="obs" label="{{__('system.obs')}}" placeholder="{{__('system.enter_obs')}}" fgroup-class="col-md-12" enable-old-support/>
                </div>
            <x-slot name="footerSlot">
        ...
        {{-- Modal EDIT --}}
        <x-adminlte-modal id="modalEdit" title=" {{ __('system.edit_construction') }}" size="lg" theme="success" icon="fas fa-store-alt" v-centered static-backdrop scrollable>
...
@section('js')
    <script>
        ...
        const edit_construction = (construction) => {
            ...
            document.querySelector('#form_edit_construction').setAttribute('action', route_edit.replace('construction_id', construction.id))
