...
@section('title', __('system.permissions'))
...
@section('content_header')
    <cw-header-title>
        ...
        @if(session('success'))
            <x-adminlte-modal id="modalMessages" title="{{__('system.warning')}}!" size="lg" theme="warning" icon="fas fa-thumbs-up" v-centered static-backdrop scrollable>

                    {!! session('success') !!}

                    <x-slot name="footerSlot">
                        <x-adminlte-button theme="warning" label="{{__('system.close')}}" data-dismiss="modal" data-toggle="modal"/>
                    </x-slot>
            </x-adminlte-modal>
    ...
    <x-adminlte-card theme="success" theme-mode="outline">
        ...
        foreach ($permission_groups as $key => $permission_group) {
            if (Auth::user()->permission_group_id === 1) {
                $data[$key]['id'] = $permission_group['id'];
                $data[$key]['name'] = $permission_group['name'];
                $data[$key]['actions'] =
                "<nobr>
                    <button class='btn btn-xs btn-default text-primary mx-1 shadow btnAction edit' title='".$system_edit."' data-id='".$permission_group["id"]."'>
                        <i class='fa fa-lg fa-fw fa-pen'></i>
                    </button>
                    <button class='btn btn-xs btn-default text-danger mx-1 shadow btnAction delete' title='".$system_delete."' data-id='".$permission_group["id"]."'>
                        <i class='fa fa-lg fa-fw fa-trash'></i>
                    </button>
                    <button class='btn btn-xs btn-default text-teal mx-1 shadow btnAction details' title='".$system_details."' data-id='".$permission_group["id"]."'>
        ...
        <x-adminlte-modal id="modalEdit" title=" {{ __('system.edit_user') }}" size="lg" theme="success" icon="fas fa-user" v-centered static-backdrop scrollable>
            <form id="form_edit_user" method="post" enctype="multipart/form-data">
                ...
                <div class="row">
                    <x-adminlte-input id="edit_input_email" type="email" name="email" label="{{__('system.email')}}" placeholder="{{__('system.enter_email')}}" fgroup-class="col-md-12" enable-old-support/>
                </div>
                <div class="row">
                    <x-adminlte-input name="password" label="{{__('system.change_password')}}" placeholder="{{__('system.if_change_password')}}" fgroup-class="col-md-12" enable-old-support/>
                </div>
            <x-slot name="footerSlot">
                <x-adminlte-button type="submit" class="mr-auto" theme="success" label="Salvar"/>
            </form>
